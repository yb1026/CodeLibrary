...
import com.cultivator.codelibrary.common.net.*;
import com.cultivator.codelibrary.common.net.NetworkConstant;
import com.cultivator.codelibrary.common.net.SSLContextManager;
import com.cultivator.codelibrary.common.net.model.BaseRequest;
import com.cultivator.codelibrary.common.util.JsonUtil;
...
public class ConnectionController {
    ...
    private void connection(int what, BaseRequest request, RequestMethod requestMethod, boolean isLoading) {
        ...
        if (requestMethod == RequestMethod.GET) {
            mRequest = NoHttp.createJsonObjectRequest(request.getUrl(), requestMethod);
            ...
            mRequest = new com.cultivator.codelibrary.common.net.FastJsonRequest(request.getUrl(), requestMethod);
            if (!request.getHeaders().isEmpty()) {
                setHeaders(request.getHeaders(), mRequest);
            }
            mRequest.setRequestBody(request.getJson().toString());
        }
        ...
        mRequest.setConnectTimeout(NetworkConstant.HTTP_TIMEOUT);
        mRequest.setReadTimeout(NetworkConstant.READ_TIMEOUT);
        mRequest.setSSLSocketFactory(SSLContextManager.getDefaultSLLContext().
                getSocketFactory());
        CallServer.getRequestInstance().add(mContext, what, mRequest, mHttpListener,
                mViewListener, true, isLoading);
    }


    public void connection(BaseRequest request, RequestMethod requestMethod, boolean isLoading) {
        MyLog.d(com.cultivator.codelibrary.common.net.ConnectionController.class, "url:" + request.getUrl());
        ...
        if (requestMethod == RequestMethod.GET) {
            ...
            MyLog.d(com.cultivator.codelibrary.common.net.ConnectionController.class, "request:" + request.getJson().toString());
    ...
    private void setHeaders(Map<String, String> map, Request<JSONObject> fjr) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!Utils.isNull(key) && !Utils.isNull(value)) {
                MyLog.d(com.cultivator.codelibrary.common.net.ConnectionController.class, "add header:" + key + "    " + value);
    ...
    private void printMap(Map<String, String> map) {
        ...
        while (iterator.hasNext()) {
            ...
            String key = entry.getKey();
            String value = entry.getValue();
            MyLog.d(com.cultivator.codelibrary.common.net.ConnectionController.class, "param:" + key + "    " + value);
        }
    }
